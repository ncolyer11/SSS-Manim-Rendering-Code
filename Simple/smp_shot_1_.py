...
class smp_shot_1(Scene):
    def construct(self):
        ...
        for group in search_shapes_in_text(compliment_formula, [MathTex(r"\text{Total Growth Chance}")], index=2):
            self.play(*[
                compliment_formula[2][group].animate.set_color(warped_stem_colour_b)
            ], run_time=1*speed) 
        ...
        for group in search_shapes_in_text(fungus_growth_equation, [MathTex(r"87.04\%")]):
            fungus_growth_equation[0][group].set_color(warped_stem_colour_b)
        self.wait()
...
# general formula
        ...
        for group in search_shapes_in_text(general_fungus_per_hour_formula, [MathTex(r"(1-0.6^{\text{Dispensers}})")]):
            general_fungus_per_hour_formula[0][group].set_color(warped_stem_colour_b)
        ...
        self.wait()
        self.play(TransformMatchingShapes(cycles_an_hour, general_fungus_per_hour_formula), run_time=1*speed)

        self.wait()
        
# finding fungi subbed in
        ...
        for group in search_shapes_in_text(fungi_an_hour, [MathTex(r"0.8704")]):
            fungi_an_hour[0][group].set_color(warped_stem_colour_b)
        self.wait()
        for group in search_shapes_in_text(fungi_an_hour, [MathTex(r"18000/ \text{h}")]):
            fungi_an_hour[0][group].set_color(YELLOW_D)
        for group in search_shapes_in_text(fungi_an_hour, [MathTex(r"15667 \text{ Fungi/h}")]):
            fungi_an_hour[0][group].set_color(green2)

        self.wait()
        self.play(TransformMatchingShapes(general_fungus_per_hour_formula, fungi_an_hour), run_time=1*speed)
